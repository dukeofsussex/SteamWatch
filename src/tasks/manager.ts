...
class Manager {
  ...
  constructor(client: CommandoClient) {
    ...
    this.newsQueue.onProcess((item: object) => processNews(client, item));
  }
  ...
  private async populateQueues() {
    const average = await avgQuery.then((res: any) => res.average);
    ...
    const apps = await db.select('app.id', 'article_id', db.raw('watcher_count + (TIMESTAMPDIFF(HOUR, IFNULL(last_checked, NOW() - INTERVAL 1 YEAR), NOW()) DIV 6) * ? AS priority', [average]))
      .from('app')
      .innerJoin(db.select('app_id', db.raw('COUNT(app_id) AS watcher_count')).from('app_watcher').groupBy('app_id').as('watchers'), 'app.id', 'watchers.app_id')
      .leftJoin(db.select('app_id', { article_id: 'id' }, db.raw('MAX(created_at) AS mca')).from('app_news').groupBy('id').as('news'), 'app.id', 'news.app_id')
      .where('last_checked', '<=', new Date().toISOString())
